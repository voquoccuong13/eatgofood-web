...
var chunkWTHM33JV_cjs = require('./chunk-WTHM33JV.cjs');
...
var _LoggerService = class _LoggerService {
  ...
  constructor(isEnabled = false, customLoggerFn) {
    ...
    this.loggerFn = chunkWTHM33JV_cjs.ignoreLogger;
    this.isEnabled = isEnabled;
    this.loggerFn = customLoggerFn || (isEnabled ? chunkWTHM33JV_cjs.consoleLogger : chunkWTHM33JV_cjs.ignoreLogger);
  }
  /**
   * Ghi log dữ liệu
   * @en Log data
...
var _PaymentService = class _PaymentService {
  ...
  buildPaymentUrl(data, options) {
    ...
    if (dataToBuild?.vnp_ExpireDate && !chunkWTHM33JV_cjs.isValidVnpayDateFormat(dataToBuild.vnp_ExpireDate)) {
      throw new Error(
        "Invalid vnp_ExpireDate format. Use `dateFormat` utility function to format it"
      );
    }
    if (!chunkWTHM33JV_cjs.isValidVnpayDateFormat(dataToBuild?.vnp_CreateDate ?? 0)) {
      const timeGMT7 = chunkWTHM33JV_cjs.getDateInGMT7();
      dataToBuild.vnp_CreateDate = chunkWTHM33JV_cjs.dateFormat(timeGMT7, "yyyyMMddHHmmss");
    }
    const redirectUrl = chunkWTHM33JV_cjs.createPaymentUrl({
      config: this.config,
      data: dataToBuild
    });
    const signed = chunkWTHM33JV_cjs.calculateSecureHash({
...
var _QueryService = class _QueryService {
  ...
  async queryDr(query, options) {
    ...
    const url = new URL(
      chunkWTHM33JV_cjs.resolveUrlString(
    ...
    const requestHashed = chunkWTHM33JV_cjs.hash(
    ...
    const message = chunkWTHM33JV_cjs.getResponseByStatusCode(
    ...
    const responseHashed = chunkWTHM33JV_cjs.hash(
      this.config.secureSecret,
    ...
    if (responseData?.vnp_SecureHash && responseHashed !== responseData.vnp_SecureHash) {
      outputResults = {
        ...outputResults,
        isVerified: false,
        message: chunkWTHM33JV_cjs.getResponseByStatusCode(
...
var _VerificationService = class _VerificationService {
  ...
  verifyReturnUrl(query, options) {
    ...
    const searchParams = chunkWTHM33JV_cjs.buildPaymentUrlSearchParams(cloneQuery);
    const isVerified = chunkWTHM33JV_cjs.verifySecureHash({
      secureSecret: this.config.secureSecret,
  ...
  verifyIpnCall(query, options) {
    ...
    const silentOptions = { logger: { loggerFn: chunkWTHM33JV_cjs.ignoreLogger } };
...
var _VNPay = class _VNPay {
  ...
  async getBankList() {
    ...
    for (const bank of bankList) {
      bank.logo_link = chunkWTHM33JV_cjs.resolveUrlString(
        this.globalConfig.vnpayHost ?? chunkEYO267YS_cjs.VNPAY_GATEWAY_SANDBOX_HOST,
...
exports.VNPay = VNPay;
//# sourceMappingURL=chunk-MVSF65JO.cjs.map
//# sourceMappingURL=chunk-MVSF65JO.cjs.map
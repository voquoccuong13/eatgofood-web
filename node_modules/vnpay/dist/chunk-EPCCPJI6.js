import { RESPONSE_MAP } from './chunk-3RXL4RP4.js';
...
__name(getDateInGMT7, "getDateInGMT7");
function dateFormat(date, format = "yyyyMMddHHmmss") {
  const pad = /* @__PURE__ */ __name((n) => (n < 10 ? `0${n}` : n).toString(), "pad");
...
__name(isValidVnpayDateFormat, "isValidVnpayDateFormat");
...
__name(getResponseByStatusCode, "getResponseByStatusCode");
...
__name(resolveUrlString, "resolveUrlString");
function hash(secret, data, algorithm) {
  return crypto2.createHmac(algorithm, secret).update(data.toString()).digest("hex");
}
__name(hash, "hash");
function ignoreLogger() {
}
__name(ignoreLogger, "ignoreLogger");
...
__name(buildPaymentUrlSearchParams, "buildPaymentUrlSearchParams");
...
__name(createPaymentUrl, "createPaymentUrl");
function calculateSecureHash({
  ...
  return crypto2.createHmac(hashAlgorithm, secureSecret).update(Buffer.from(data, bufferEncode)).digest("hex");
}
__name(calculateSecureHash, "calculateSecureHash");
function verifySecureHash({
  ...
  const calculatedHash = crypto2.createHmac(hashAlgorithm, secureSecret).update(Buffer.from(data, "utf-8")).digest("hex");
  return calculatedHash === receivedHash;
}
__name(verifySecureHash, "verifySecureHash");
...
//# sourceMappingURL=chunk-EPCCPJI6.js.map
//# sourceMappingURL=chunk-EPCCPJI6.js.map